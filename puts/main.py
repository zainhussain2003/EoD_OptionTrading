...
Systematic MWF ATM put trading: find the optimal entry/exit time, trade every week.
...
def _fallback_contracts(ticker, spot, lower, upper, expiry, config):
    ...
    from utils.math_utils import black_scholes_put, calc_theta_hourly
    ...
    for strike in (lower, upper):
        ...
        price, delta, _, _ = black_scholes_put(spot, strike, T, config.risk_free_rate, sigma)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description='EoD Options Analyzer — systematic MWF ATM put trading'
    )
