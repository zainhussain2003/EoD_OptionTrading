...
def get_past_thursday_dates(days_back: int = 730) -> list[date]:
    """All Thursday dates in the past `days_back` calendar days, oldest first.

    Includes every calendar Thursday in the window; the backtester marks a
    closed Thursday (e.g. Thanksgiving) as a missing week for provenance.
    """
    today = date.today()
    result = []
    for i in range(1, days_back + 1):
...
def weekly_expiry_for(thursday: date) -> date:
    """The weekly expiry for a Thursday entry.

    The weekly normally expires the following FRIDAY (so a Thursday trade is
    1DTE intraday). When that Friday is a market holiday (e.g. Good Friday) the
    weekly expires the THURSDAY itself, making it a 0DTE trade.
    """
    friday = thursday + timedelta(days=1)
    return friday if is_trading_day(friday) else thursday
