#!/usr/bin/env python
"""
PIPELINE ADAPTER for the THURSDAY INTRADAY optimal entry/exit time-frame study
(AAPL & TSLA, Friday-expiry weekly traded intraday on Thursday, calls & puts).
...
and emits the pipeline's output contract:
  ...
  2. Writes all artifacts into STRATEGY_RESULTS_DIR (falls back to ./output):
       ...
       - trades.csv        per-Thursday ledger (both calls and puts)
...
def save_artifacts(out_dir, all_results, outlier_max=None):
    ...
    try:
        ...
        if plotted:
            ...
            ax.set_title("Cumulative P&L — Thursday intraday optimal entry/exit "
                         "frames (AAPL & TSLA)")
            ax.set_xlabel("Trade # (Thursdays)")
...
def build_metrics(all_results, lookback_days, target_spend, outlier_max):
    ...
    metrics = {
        "status": "ok",
        "strategy": "thursday-intraday",
        "tickers": ["AAPL", "TSLA"],
        "entry_day": "Thursday (intraday — buy and sell the same session)",
        "expiry": "Friday weekly (1DTE; 0DTE only when Friday is a holiday)",
...
def print_summary(metrics):
    ...
    print("\n" + "=" * 68)
    print("  STRATEGY SUMMARY — Thursday intraday optimal entry/exit (AAPL & TSLA)")
    print("=" * 68)
    ...
    print("  Best intraday entry/exit frame per leg (buy → sell, same Thursday):")
...
def main() -> int:
    ...
    try:
        ...
        all_results = te.run_timeframe(
            ...
            eligible=eligible,
            size_fn=size_fn,
            file_tag="thursday_intraday_timeframe",
            outlier_max=OUTLIER_MAX,
            header_extra=(
        ...
        metrics = {"status": "error", "strategy": "thursday-intraday",
