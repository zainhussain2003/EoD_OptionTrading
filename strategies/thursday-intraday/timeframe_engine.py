#!/usr/bin/env python3
"""
Time-frame search engine for the Thursday intraday optimal entry/exit study
(AAPL & TSLA, Friday-expiry weekly traded intraday on Thursday).
...
def print_heatmaps(ticker, opt_type, cell_best, optimal_cell):
    ...
    _print_grid("3b) TOTAL P&L  ($ per contract, summed over all Thursdays)",
...
def save_csv(opt_type, method_label, header_extra, rows, summ, frame, ranked,
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        # Per-Thursday detail
        w.writerow(["DETAIL — one row per Thursday (ATM "
