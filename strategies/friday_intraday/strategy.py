#!/usr/bin/env python
"""
PIPELINE ADAPTER for the FRIDAY INTRADAY optimal entry/exit time-frame study
(0DTE Friday-expiry weeklies across TSLA, AMZN, AAPL, NVDA, GOOGL, MSFT, ORCL —
calls & puts).
...
def print_summary(metrics):
    ...
    print("\n" + "=" * 70)
    print("  STRATEGY SUMMARY — Friday intraday optimal entry/exit (0DTE, 7 names)")
    print("=" * 70)
    ...
    print("  Best intraday entry/exit frame per leg (buy → sell, same Friday):")
...
def main() -> int:
    ...
    try:
        ...
        all_results = te.run_timeframe(
            ...
            eligible=eligible,
            size_fn=size_fn,
            file_tag="friday_intraday_timeframe",
            outlier_max=OUTLIER_MAX,
            header_extra=(
        ...
        metrics = {"status": "error", "strategy": "friday_intraday",
