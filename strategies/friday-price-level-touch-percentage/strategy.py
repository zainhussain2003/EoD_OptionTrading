...
def build_metrics(all_results, config, source_label):
    ...
    Each ticker's hit-rate references are nested under its percentage thresholds:
    tickers_detail[ticker]["thresholds_pct"][pct_str]["references"][label]. The
    swing figures are threshold-independent, so they live once under
    tickers_detail[ticker]["swings"][label]."""
        ...
        for pct in res["pcts"]:
            ...
            thresholds_pct[f"{pct:.4f}"] = {
                ...
                "references": {lab: _ref_block(summaries[lab])
                               ...
                               if summaries[lab]["n"] > 0},
            }
        swing_view = res["swing_summaries"]
        swings = {
            lab: {
                "avg_up_swing_pct": round(swing_view[lab]["avg_up_swing_pct"], 6),
                "avg_down_swing_pct": round(swing_view[lab]["avg_down_swing_pct"], 6),
                "max_up_swing_pct": round(swing_view[lab]["max_up_swing_pct"], 6),
                "max_down_swing_pct": round(swing_view[lab]["max_down_swing_pct"], 6),
                "avg_up_swing": round(swing_view[lab]["avg_up_swing"], 4),
                "avg_down_swing": round(swing_view[lab]["avg_down_swing"], 4),
                "max_up_swing": round(swing_view[lab]["max_up_swing_at_pct"], 4),
                "max_down_swing": round(swing_view[lab]["max_down_swing_at_pct"], 4),
            }
            for lab in res["ref_labels"] if swing_view[lab]["n"] > 0
        }
        detail[ticker] = {
            "pcts": res["pcts"],
    ...
    return {
        "status": "ok",
        "strategy": "friday-price-level-touch-percentage",
        "tickers": tickers,
...
def save_artifacts(out_dir, all_results):
    ...
    # swings.csv — fixed-name copy of the per-Friday × per-reference × pct ledger.
    ...
    try:
        ...
        if plot_tickers:
            n = len(plot_tickers)
            ncols = max(len(all_results[t]["pcts"]) for t in plot_tickers)
            fig, axes = plt.subplots(n, ncols,
                                     figsize=(4.0 * ncols, 2.6 * n), squeeze=False)
            for row, ticker in enumerate(plot_tickers):
                ...
                for col in range(ncols):
                    ...
                    labels = [lab for lab in res["ref_labels"]
                              if summaries[lab]["n"] > 0]
                    ups = [summaries[l]["up_rate"] * 100 for l in labels]
                    dns = [summaries[l]["down_rate"] * 100 for l in labels]
                    x = range(len(labels))
                    ax.bar([i - 0.2 for i in x], ups, width=0.4,
                           label=f"+{pct:.1%} hit %", color="#2ca02c")
                    ax.bar([i + 0.2 for i in x], dns, width=0.4,
                           label=f"-{pct:.1%} hit %", color="#d62728")
                    ax.set_xticks(list(x))
                    ax.set_xticklabels(labels, fontsize=7, rotation=45, ha="right")
                    if col == 0:
                        ax.set_ylabel("hit %")
                    ax.set_title(f"{ticker}  (±{pct:.1%})", fontsize=9)
                    ax.legend(fontsize=7)
            fig.suptitle("Friday touch rate by Thursday reference time "
                         "and percentage threshold", y=1.0)
            fig.tight_layout()
...
def print_summary(metrics):
    print("\n" + "=" * 60)
    print("  STRATEGY SUMMARY — Friday price-level-touch (percentage)")
    print("=" * 60)
    ...
    for ticker, d in metrics.get("tickers_detail", {}).items():
        ...
        print(f"  {ticker:>6}: {d['n_fridays']} Fridays   best ref {rec}")
        for pct_str, block in d.get("thresholds_pct", {}).items():
...
def main() -> int:
    ...
    try:
        ...
        metrics = {"status": "error", "strategy": "friday-price-level-touch-percentage",
