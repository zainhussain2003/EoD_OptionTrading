...
For every past Friday in the lookback window, and for each ticker:
  ...
  3. TOUCH / SWING — for each reference price R and each percentage threshold pct
     in the ticker's list (e.g. 2%, 2.5%, 3%):
        ...
        max_up_swing   = fri_high - R   (also as a % of R: (fri_high - R) / R)
        max_down_swing = R - fri_low    (also as a % of R: (R - fri_low) / R)

     The max-up / max-down swing is the biggest distance Friday travelled from R
     in each direction that day; it depends only on R and Friday's high/low, not
     on pct, so it is identical across a ticker's percentage thresholds.
...
class LevelTouchAnalyzer:
    ...
    def analyze(self, ticker: str) -> dict:
        ...
        print(f"  Analyzing {ticker} (thresholds ±{pct_str})...", flush=True)
        ...
        for friday in get_past_friday_dates(self.config.backtest_days):
            ...
            for label in ref_labels:
                ...
                # Friday made from R in each direction (dollars and as a % of R).
                up_swing = fri_high - R
                down_swing = R - fri_low
                ...
                for pct in pcts:
                    ...
                    records.append(FridayLevelRecord(
                        ...
                        thu_ref_price=round(R, 4), pct=pct,
