#!/usr/bin/env python3
"""
Time-frame search engine for the Friday intraday optimal entry/exit study
(0DTE Friday weeklies across TSLA, AMZN, AAPL, NVDA, GOOGL, MSFT, ORCL).
...
and a single-transaction "concentration capped" second pass (a frame is eligible
only if its biggest single winning trade is <= a share of its net total P&L).
Calls and puts are both run and each writes its own CSV + TXT
(+ *_concentration_capped.txt) into backtest_results/.
"""
...
# ── scoring ─────────────────────────────────────────────────────────────────
def pair_stat(payoffs: list):
    """Win-rate / avg / total over a frame pair's per-Friday payoffs (per share,
    already scaled by contracts).

    Also carries the single-transaction concentration figures so the cap can
    decide eligibility without a separate pass (replaces the old outlier system):
      max_win            — largest single winning trade P&L (same sized units)
      single_trade_share — max_win / total when total > 0 and a win exists,
                           else None. This is exactly what the cap tests.
    """
    pl = payoffs
    ...
    total = sum(pl)
    win_vals = [p for p in pl if p > 0]
    max_win = max(win_vals) if win_vals else None
    share = (max_win / total) if (max_win is not None and total > 0) else None
    return {"wins": wins, "n": n, "wr": wins / n,
            "avg": total / n, "total": total,
...
def coarse_search(index_by_date, size_fn, score_key, eligible):
...
def refine_search(index_by_date, best, size_fn, score_key, eligible):
...
def summarize(rows: list) -> dict:
    ...
    single_trade_share is the biggest single winning trade as a fraction of the
    net total P&L (None when total <= 0 or there are no winners) — the figure the
    concentration cap tests, reported so it is visible the cap held.
    """
    ...
    if not traded:
        return {"n": 0, "wins": 0, "win_rate": 0.0, "total_pnl": 0.0,
                ...
                "n_skipped": len(rows), "max_win": 0.0, "single_trade_share": None}
    pnls = [r["pnl_dollars"] for r in traded]
    wins = sum(1 for p in pnls if p > 0)
    total = sum(pnls)
    win_vals = [p for p in pnls if p > 0]
    max_win = max(win_vals) if win_vals else 0.0
    share = (max_win / total) if (win_vals and total > 0) else None
...
# ── orchestration ───────────────────────────────────────────────────────────
def _analyze(opt_type, records, size_fn, score_key, eligible, eligible_capped):
    """Run the full coarse→fine search for one option type. Returns a dict of
    everything the printers and CSV need (unconstrained pass + concentration-
    capped pass).

    Pass 1 (unconstrained) ranks every frame by the score metric with no cap.
    Pass 2 (concentration-capped) re-runs the SAME search but only among frames
    whose biggest single winning trade is <= the cap fraction of the frame's net
    total P&L — no trade is dropped, the cap only constrains which frame wins. A
    frame that beats the cap only because one jackpot trade carries it is thereby
    disqualified, so pass 2 exposes whether the edge is broad-based or luck.
    """
    ...
    # Pass 2: concentration-capped — re-optimize among cap-eligible frames only.
    if eligible_capped is not None:
        b2, r2, c2 = coarse_search(
            index_by_date, size_fn, score_key, eligible_capped)
        f2 = refine_search(index_by_date, b2, size_fn, score_key, eligible_capped)
        rows2 = (per_day_pnl(opt_type, reps, meta, index_by_date, f2, size_fn)
                 if f2 else [])
        out["excl"] = {"frame": f2, "rows": rows2, "summ": summarize(rows2),
                       "ranked": r2, "cell_best": c2}
    return out
...
def run_timeframe(lookback_days, method_label, score_key, eligible, size_fn,
                  file_tag, eligible_capped=None, cap_share=None, header_extra=""):
    ...
    eligible_capped, when given, drives a second concentration-capped pass; the
    frame it chooses is the best one whose biggest single winning trade is at most
    `cap_share` of its net total P&L. cap_share is only used for labels."""
    ...
    for ticker in config.tickers:
        for opt_type in config.option_types:
            ...
            cap_pct = f"{cap_share:.0%}" if cap_share is not None else "cap"
            ...
            def _print_excl():
                ex = res["excl"]
                print(bold("\n" + "█" * 86))
                print(bold(f"  CONCENTRATION CAPPED (biggest single win <= {cap_pct} "
                           f"of total P&L)  —  {ticker} {opt_word(opt_type)}S "
                           f"re-optimized"))
                print(bold("█" * 86))
                share = ex["summ"].get("single_trade_share")
                if ex["frame"] is None:
                    print(yellow(f"  (no frame satisfied the {cap_pct} concentration "
                                 f"cap — every candidate is carried by one trade)"))
                elif share is not None:
                    print(yellow(f"  (chosen frame's biggest single win = "
                                 f"{share:.0%} of its total P&L)"))
            ...
            if eligible_capped is not None:
                ...
                with contextlib.redirect_stdout(excl_buf):
                    _print_excl()
                excl_path = os.path.join(RESULTS_DIR,
                                         f"{base}_concentration_capped.txt")
                excl_hdr = (
                    f"TIME-FRAME BACKTEST — {method_label} ({opt_word(opt_type)}s)"
                    f"   [CONCENTRATION CAPPED <= {cap_pct} single-trade share]\n"
                    f"Re-optimized among frames only where the biggest single\n"
                    f"winning trade is at most {cap_pct} of the frame's net total\n"
                    f"P&L (no trade dropped; the cap constrains frame selection).\n"
                    f"Lookback = {lookback_days} calendar days\n"
                    f"Session window = {frame_label(DAY_START_M, DAY_END_M)} ET\n"
                    f"Generated {datetime.now():%Y-%m-%d %H:%M:%S}  |  {source_label}\n"
                    f"P&L $ assumes 1 contract = {CONTRACT_MULTIPLIER} shares\n"
            ...
            if excl_path:
                print(f"  {cyan('Saved TXT (concentration capped):')} {bold(excl_path)}")
            print(bold("─" * 86))
            print()
