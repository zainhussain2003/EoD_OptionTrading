...
def get_strike_at_or_below(spot: float, interval: float | None = None) -> float:
    """Largest standard strike that is <= spot — the put you'd actually buy.

    The put mirror of get_strike_at_or_above: the at-the-money strike when spot
    sits exactly on one, otherwise the first strike BELOW spot. Never returns a
    strike above spot. Examples (interval 2.5): spot 400.25 -> 400.0;
    spot 400.00 -> 400.0.
    """
    if interval is None:
        interval = detect_strike_interval(spot)
    n = math.floor(round(spot / interval, 6))
    return round(n * interval, 2)
