#!/usr/bin/env python3
"""
Shared engine for the THURSDAY → FRIDAY option probability backtest, for both
CALLS (bet the stock rises) and PUTS (bet it falls).

Strategy modelled
-----------------
  • The weekly expiry is FRIDAY. You buy the ATM option the trading day BEFORE
    expiry — normally THURSDAY — between 3:55 and 3:59 PM ET (minute by minute).
      - CALL: strike at-or-above spot (never below).  You want the stock UP.
      - PUT : strike at-or-below spot (never above).  You want the stock DOWN.
  ...
  • The question: what fraction of the time did the option's expiry-day session
    HIGH reach a target return multiple of your entry price — and what does a
    limit-at-target-else-sell-at-3:55 strategy actually net?
...
"reaching" = the expiry-day intraday HIGH of the OPTION touches the target at
any point.

Data
----
Real Alpaca option bars when available; Black-Scholes reconstruction (from the
underlying's path + realized vol) as a fallback. A call's option-high is driven
by the stock's HIGH; a put's option-high is driven by the stock's LOW. Both legs
of a sample (the entry-day price and the expiry-day path) always come from the
SAME source, so the return ratio is never real-vs-simulated apples-to-oranges.
"""
...
from utils.date_utils import (
    ...
    get_strike_at_or_above, get_strike_at_or_below, format_contract_symbol,
    minute_to_str, is_trading_day,
)
...
def _bs_price(opt_type: str, spot: float, strike: float, T: float, r: float,
              sigma: float) -> float:
    """Black-Scholes price for the requested option type."""
    if opt_type == "P":
        px, *_ = black_scholes_put(spot, strike, T, r, sigma)
    else:
...
# ── one (entry-day → expiry-day, ticker, opt_type) sample ─────────────────
def _build_sample(fetcher, ticker, opt_type, entry_d, expiry_d, strikes, r, sigma):
    """Return a sample dict or None. Tries REAL bars first, then BS simulation.

    Sample = {entry_date, expiry_date, ticker, opt_type, strike, contract, source,
              exp_high, exp_high_to_exit, exp_exit_price, entries: {minute: price}}
    exp_high          — full-session (9:30–4:00) high of the OPTION, drives the
                        touch table.
    exp_high_to_exit  — option high up to EXIT_MINUTE, drives the strategy fill.
    exp_exit_price    — option price at EXIT_MINUTE, the strategy's fallback sell.
    Both legs share one source so the return ratio stays internally consistent.

    For a PUT the option's value rises as the stock falls, so in the simulation
    path the option HIGH is driven by the stock LOW (not the stock high).
    """
    ...
    # The option's own HIGH already captures direction, so calls and puts share
    # this branch unchanged — only the OCC symbol's C/P and the strike differ.
    for strike in strikes:
        contract = format_contract_symbol(ticker, expiry_d, strike, opt_type)
        ...
        if entries and x_high_map:
            return {
                ...
                "opt_type": opt_type, "strike": strike, "contract": contract,
                "source": SOURCE_REAL,
    ...
    for m, spot in e_sclose.items():
        ...
        px = _bs_price(opt_type, spot, strike, T, r, sigma)
        if px > 0:
            entry_opt[m] = px
    entries = {}
    for m in ENTRY_MINUTES:
    ...
    def _bs_at(spot, m):
        ...
        T = _T_years(et, expiry_d)
        return _bs_price(opt_type, spot, strike, T, r, sigma)

    # Direction of the option's max: calls peak at the stock HIGH, puts at LOW.
    peak_field = "low" if opt_type == "P" else "high"
    x_speak = _bars_minute_field(x_stock, expiry_d, peak_field,
                                 SESSION_OPEN_MIN, SESSION_CLOSE_MIN)
    x_sclose = _bars_minute_field(x_stock, expiry_d, "close",
                                  SESSION_OPEN_MIN, SESSION_CLOSE_MIN)
    high_full = [_bs_at(spot, m) for m, spot in x_speak.items()]
    high_exit = [_bs_at(spot, m) for m, spot in x_speak.items() if m <= EXIT_MINUTE]
    close_opt = {m: _bs_at(spot, m) for m, spot in x_sclose.items()}
    if not high_full:
        return None
...
def collect_samples(fetcher, config, pairs):
    """Gather one sample per (ticker, opt_type, entry→expiry) pair.

    Returns {(ticker, opt_type): [samples]}.
    """
    by_key = defaultdict(list)
    for ticker in config.tickers:
        # Realized vol for any BS simulation on this ticker (shared C/P).
        closes = []
        ...
        for opt_type in config.option_types:
            print(f"  Collecting {ticker} {OPT_LABEL[opt_type]}s...", flush=True)
            for entry_d, expiry_d in pairs:
                # Pick the ATM strike from the entry day's late-afternoon spot.
                ...
                # CALL: strike at/above spot (ATM or first OTM call, never below).
                # PUT : strike at/below spot (ATM or first OTM put, never above).
                # The second entry is a data-availability fallback (one strike
                # further OTM in the same direction) for weeks with no bars.
                if opt_type == "P":
                    base = get_strike_at_or_below(spot, interval)
                    strikes = [base, round(base - interval, 2)]
                else:
                    base = get_strike_at_or_above(spot, interval)
                    strikes = [base, round(base + interval, 2)]

                sample = _build_sample(fetcher, ticker, opt_type, entry_d, expiry_d,
                                       strikes, config.risk_free_rate, sigma)
                if sample is not None:
                    sample["spot"] = spot
                    by_key[(ticker, opt_type)].append(sample)
...
def _print_touch_table(samples, multiples):
    """Probability the expiry-day option HIGH reached each target (best-case touch)."""
    print(f"  TOUCH PROBABILITY — chance the expiry-day option high reached the target")
...
def _pool(by_key, opt_type=None):
    pooled = []
    for (ticker, ot), samples in by_key.items():
        if opt_type is None or ot == opt_type:
            pooled.extend(samples)
    return pooled


def _print_all(by_key, config, multiples):
    for ticker in config.tickers:
        for opt_type in config.option_types:
            _print_block(f"{ticker} {OPT_LABEL[opt_type]}S",
                         by_key.get((ticker, opt_type), []), multiples)
    for opt_type in config.option_types:
        _print_block(f"ALL TICKERS — {OPT_LABEL[opt_type]}S COMBINED",
                     _pool(by_key, opt_type), multiples, show_source=False)
...
def _save_csv(path, method_label, lookback_days, source_label, multiples,
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        w.writerow(["ticker", "opt_type", "entry_day", "entry_weekday", "expiry_day",
        ...
        for ticker in config.tickers:
            for opt_type in config.option_types:
                for s in by_key.get((ticker, opt_type), []):
                    for minute in ENTRY_MINUTES:
                        ep = s["entries"].get(minute)
                        if ep is None:
                            continue
                        ...
                        w.writerow([ticker, opt_type, s["entry_date"],
                                    wd[s["entry_date"].weekday()], s["expiry_date"],
                                    wd[s["expiry_date"].weekday()], s["strike"],
                                    s["contract"], src, minute_to_str(minute),
                                    round(ep, 4), round(fh, 4), round(max_ret, 4)]
                                   + [int(fh >= ep * (1.0 + m)) for m in multiples])
        w.writerow([])

        # ---- SUMMARY: probabilities per scope × entry minute ----
        w.writerow(["SUMMARY — touch probability by scope and entry minute"])
        w.writerow(["scope", "opt_type", "entry_time", "n_samples", "avg_entry_price",
                    "avg_max_ratio", "best_max_ratio"]
                   + [f"prob_{m:g}x" for m in multiples])
        scopes = []
        for ticker in config.tickers:
            for opt_type in config.option_types:
                scopes.append((ticker, opt_type, by_key.get((ticker, opt_type), [])))
        for opt_type in config.option_types:
            scopes.append(("ALL", opt_type, _pool(by_key, opt_type)))
        for name, opt_type, samples in scopes:
            for minute in ENTRY_MINUTES:
                st = minute_stats(samples, minute, multiples)
                if st is None:
                    w.writerow([name, opt_type, minute_to_str(minute), 0, "", "", ""]
                               + ["" for _ in multiples])
                    continue
                w.writerow([name, opt_type, minute_to_str(minute), st["n"],
        ...
        w.writerow(["scope", "opt_type", "entry_time", "target_multiple", "n_samples",
        ...
        for name, opt_type, samples in scopes:
            for minute in ENTRY_MINUTES:
                for m in multiples:
                    ss = strategy_stats(samples, minute, m)
                    if ss is None:
                        w.writerow([name, opt_type, minute_to_str(minute), f"{m:g}x",
                                    0, "", "", "", "", ""])
                        continue
                    w.writerow([name, opt_type, minute_to_str(minute), f"{m:g}x", ss["n"],
...
# ── driver ───────────────────────────────────────────────────────────────
def run(lookback_days, multiples,
    ...
    print(f"  Option types: {', '.join(OPT_LABEL[o] for o in config.option_types)}")
    ...
    by_key = collect_samples(fetcher, config, pairs)

    print()
    _print_all(by_key, config, multiples)
    ...
    base = f"thu_fri_aapl_tsla_{lookback_days}days_{stamp}"
