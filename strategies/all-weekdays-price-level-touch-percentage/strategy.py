...
def build_metrics(all_results, config, source_label):
    ...
    return {
        "status": "ok",
        "strategy": "all-weekdays-price-level-touch-percentage",
        "tickers": tickers,
...
def save_artifacts(out_dir, all_results):
    ...
    try:
        ...
        import level_touch_engine as le   # for pct_label (exact 0.75% / 1.25%)
        ...
        if plot_tickers:
            ...
            for row, ticker in enumerate(plot_tickers):
                res = all_results[ticker]
                pcts = res["pcts"]
                xlabels = [le.pct_label(p) for p in pcts]
                x = range(len(pcts))
                for col, day_name in enumerate(days):
                    ...
                    # 14 rungs per panel collide when drawn flat, and the dense
                    # low end (0.5/0.75/1/1.25%) is the worst of it — rotate so
                    # every threshold stays readable.
...
def print_summary(metrics):
    print("\n" + "=" * 66)
    print("  STRATEGY SUMMARY — all-weekday price-level-touch (percentage)")
    print("=" * 66)
    ...
    try:
        ...
        all_results = le.run_level_touch(
            ...
            header_extra=(
                ...
                "across the 14-level ladder (0.5% → 15%), for all five weekdays; "
                "swings shown as % of reference ($ in brackets)."
            ),
        )
        ...
        metrics = {"status": "error", "strategy": "all-weekdays-price-level-touch-percentage",
