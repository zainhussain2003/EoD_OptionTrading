#!/usr/bin/env python3
"""
Engine for the all-weekday price-level-touch (percentage) study.
...
def pct_label(pct: float) -> str:
    """Threshold as a compact exact label: 0.005 → '0.5%', 0.0075 → '0.75%',
    0.01 → '1%'. A plain `.1%` format would round the v2 quarter-point levels
    (0.75% → '0.8%', 1.25% → '1.2%'), so thresholds are labelled with this
    instead; hit RATES still use `.1%`, where one decimal is the right precision."""
    return f"{pct * 100:g}%"
...
def print_hit_rate_table(day_res: dict, ref_labels: list, pct: float):
    ...
    pos = f"+{pct_label(pct)} hit"
    neg = f"-{pct_label(pct)} hit"
    print(bold(f"     ── ±{pct_label(pct)} level ──"))
...
def save_touch_summary_csv(path, all_results, lookback_days, source_label):
    with open(path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["# All-weekday price-level-touch (percentage) — touch summary"])
...
def save_swings_csv(path, all_results, lookback_days, source_label):
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["# All-weekday price-level-touch (percentage) — swing ledger"])
        w.writerow([f"# Lookback = {lookback_days} calendar days  |  "
                    f"data source: {source_label}"])
        w.writerow([f"# Generated {datetime.now():%Y-%m-%d %H:%M:%S}"])
        w.writerow([])
...
def save_ticker_txt(path, result, lookback_days, source_label):
    ...
    hdr = (f"ALL-WEEKDAY PRICE-LEVEL-TOUCH (PERCENTAGE) — {result['ticker']} "
...
def run_level_touch(lookback_days=None, header_extra=""):
    """Capture prior-day baselines + scan-day sessions per ticker across every scan day,
    ...
    print(bold("\n" + "═" * 92))
    print(bold("  ALL-WEEKDAY PRICE-LEVEL-TOUCH ANALYSIS (PERCENTAGE)"))
