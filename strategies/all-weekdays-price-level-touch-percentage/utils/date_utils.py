...
def get_past_friday_dates(days_back: int = 730) -> list[date]:
    ...
    return get_past_weekday_dates(days_back, 4)
...
def get_past_weekday_dates(days_back: int, weekday: int) -> list[date]:
    """All dates matching `weekday` (Mon=0 … Fri=4) in the past `days_back`
...
def prior_trading_weekday(target: date) -> date:
    """The trading day immediately before `target`, skipping weekends. Mon→prior
    Fri, Wed→Tue, Fri→Thu. (Holidays are handled separately by the baseline
    walk-back, which steps further back when this day has no data.)"""
