...
class FridayLevelRecord:
    ...
    reference price (e.g. the 3:52 PM close)."""
    date: str
    ticker: str
    thu_ref_label: str        # '3:52 PM' or '3:50–55 avg'
    thu_ref_price: float
    threshold: float
    up_level: float           # thu_ref_price + threshold
    down_level: float         # thu_ref_price - threshold
    fri_high: float           # Friday session high (max of 1-min bar highs)
    fri_low: float            # Friday session low  (min of 1-min bar lows)
    max_up_swing: float       # fri_high - thu_ref_price
    max_down_swing: float     # thu_ref_price - fri_low
