# Mon/Wed/Fri Price-Level-Touch Analysis (Percentage) — v2
...
This is a **fresh-data re-run** of `mon-wed-fri-price-level-touch-percentage`,
which last ran **2026-06-28** over a 730-day window (≈2024-06-28 → 2026-06-28).
v2 keeps the same method, lookback and baseline definition, and changes only the
basket and the threshold ladder:

| | v1 (2026-06-28) | v2 |
|---|---|---|
| Tickers | TSLA, AAPL, NVDA, MSFT, ORCL | **TSLA, AAPL** |
| Thresholds | 1%, 1.5%, 2%, 2.5%, 3% (5) | **0.5%, 0.75%, 1%, 1.25%, 1.5%, 2%, 2.5%, 3%, 3.5%, 4%, 5% (11)** |
| Lookback | 730 days, rolling | 730 days, rolling (window slides forward) |
| Scan days | Mon / Wed / Fri | Mon / Wed / Fri |
| Baseline | prior day 3:50–3:55 PM | prior day 3:50–3:55 PM |

The ladder **retains all five v1 levels**, so v2 rows can be read directly against
the v1 run; the six additions fill in both ends of the curve:

- **0.5% / 0.75% / 1.25%** — dense sampling of the steep region, where the hit
  rate falls fastest (this is where AAPL lives).
- **3.5% / 4% / 5%** — the tail, which realistically only TSLA reaches.

Both tickers deliberately share one ladder: TSLA saturates near 100% at the low
end and AAPL decays toward 0% at the high end, so each name still gets a **full
curve** and the two can be read on a single x-axis.

Because the 730-day window is **rolling**, it slides forward rather than
extending — v2 adds the sessions since the v1 run and drops an equal number of
the oldest ones. It is a 2-year window as of the run date, not a superset of v1.
...
- **Touch detection (intraday high/low).** For each baseline price `R` and each
  percentage `pct` on the ladder (0.5% → 5%):
...
- **Per scan day (Monday → Wednesday → Friday):**
  - **Hit-rate sub-tables — one per percentage (11 of them, 0.5% → 5%)** — per
