...
def check_regimes(instances: pd.DataFrame, cfg) -> dict:
    """Daily names should show <DOW>-0DTE; ORCL/AMD early-week entries bridge."""
    problems = []
    if instances.empty:
        return {"problems": ["no instances"], "ok": False}
    seen = instances.groupby("symbol")["is_0dte"].agg(["mean", "count"])
    for sym in cfg.daily_0dte_names:
        if sym in seen.index and seen.loc[sym, "mean"] < 0.5:
            problems.append(f"{sym}: expected mostly 0DTE, saw {seen.loc[sym,'mean']:.2f}")
    for sym in cfg.friday_weekly_names:
        sub = instances[instances["symbol"] == sym]
        if not sub.empty:
            early = sub[sub["regime"].str.contains("->")]
            if early.empty:
                problems.append(f"{sym}: expected some bridge (Mon->Fri) labels, found none")
    return {"problems": problems, "ok": len(problems) == 0,
            "per_symbol_0dte_share": seen["mean"].round(3).to_dict()}
