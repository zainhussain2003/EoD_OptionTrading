...
def build_summary_md(ctx: dict) -> str:
    cfg = ctx["cfg"]
    L = []
    L.append("# 0DTE / short-dated option threshold-timing — summary\n")
    ...
    # ── ORCL / AMD note ──────────────────────────────────────────────────────
    L.append("## ORCL / AMD — multi-day holds, NOT 0DTE\n")
    L.append("ORCL and AMD list Friday weeklies only, so early-week entries bridge to "
             "Friday (labelled e.g. `Mon->Fri`, with days-to-expiry recorded). Their "
             "threshold timing reflects multi-day theta/vega dynamics and must not be "
             "compared directly with the true 0DTE names.\n")
