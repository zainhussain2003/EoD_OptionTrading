#!/usr/bin/env python
"""
PIPELINE ADAPTER for the TUESDAY INTRADAY optimal entry/exit time-frame study
(Magnificent 7, Wednesday-expiry option traded intraday on Tuesday, calls & puts).
...
and emits the pipeline's output contract:
  ...
  2. Writes all artifacts into STRATEGY_RESULTS_DIR (falls back to ./output):
       ...
       - trades.csv        per-Tuesday ledger (both calls and puts)
...
def save_artifacts(out_dir, all_results, outlier_max=None):
    ...
    try:
        ...
        if plotted:
            ...
            ax.set_title("Cumulative P&L — Tuesday intraday optimal entry/exit "
                         "frames (Mag 7)")
            ax.set_xlabel("Trade # (Tuesdays)")
...
def _pass_metrics(all_results, pass_key):
    ...
    A leg is a (ticker × call/put) pair, keyed e.g. "AAPL_CALL" — so AAPL and
    TSLA keep their own optimal frames instead of one overwriting the other.
...
def build_metrics(all_results, lookback_days, target_spend, outlier_max):
    ...
    metrics = {
        "status": "ok",
        "strategy": "tuesday_intraday",
        "tickers": ["AAPL", "MSFT", "GOOGL", "AMZN", "NVDA", "META", "TSLA"],
        "entry_day": "Tuesday (intraday — buy and sell the same session)",
        "expiry": "Wednesday (1DTE; 0DTE only when Wednesday is a holiday). Real "
                  "Wednesday-expiry bars when available, else Black-Scholes sim",
...
def print_summary(metrics):
    ...
    print("\n" + "=" * 68)
    print("  STRATEGY SUMMARY — Tuesday intraday optimal entry/exit (Mag 7)")
    print("=" * 68)
    ...
    print("  Best intraday entry/exit frame per leg (buy → sell, same Tuesday):")
    for key, leg in legs.items():
        print(f"    {key:<11}: enter {str(leg.get('entry_frame')):>15}  "
...
def main() -> int:
    ...
    try:
        ...
        all_results = te.run_timeframe(
            ...
            eligible=eligible,
            size_fn=size_fn,
            file_tag="tuesday_intraday_timeframe",
            outlier_max=OUTLIER_MAX,
            header_extra=(
        ...
        metrics = {"status": "error", "strategy": "tuesday_intraday",
