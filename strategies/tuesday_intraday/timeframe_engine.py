#!/usr/bin/env python3
"""
Time-frame search engine for the Tuesday intraday optimal entry/exit study
(Magnificent 7, Wednesday-expiry option traded intraday on Tuesday).
...
def print_heatmaps(ticker, opt_type, cell_best, optimal_cell):
    ...
    _print_grid("3b) TOTAL P&L  ($ per contract, summed over all Tuesdays)",
...
def save_csv(opt_type, method_label, header_extra, rows, summ, frame, ranked,
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        # Per-Tuesday detail
        w.writerow(["DETAIL — one row per Tuesday (ATM "
