#!/usr/bin/env python3
"""
TUESDAY INTRADAY — OPTIMAL ENTRY/EXIT TIME-FRAME ANALYSIS  (target-spend sized)

Finds the best *time frame* to ENTER and the best *time frame* to EXIT a
Magnificent-7 option over the whole regular session (9:30 AM–4:00 PM ET), on
TUESDAYS only, trading the WEDNESDAY-expiry option intraday (1DTE — bought and
sold the same Tuesday), for BOTH calls and puts.

Note: individual equities often list only Friday weeklies, so a real
Wednesday-expiry contract may not exist for many Tuesdays — those fall back to a
Black-Scholes simulation (real Alpaca bars are used whenever they are available).
...
LOOKBACK_DAYS = 150          # <───── window to test (calendar days)
