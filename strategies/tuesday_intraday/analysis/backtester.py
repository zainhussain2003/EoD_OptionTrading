...
from utils.date_utils import (
    ET, get_past_tuesday_dates, weekly_expiry_for, is_trading_day,
...
class Backtester:
    ...
    TUESDAY, for BOTH the ATM call and the ATM put, trading the WEDNESDAY-expiry
    option (1 day to expiry — 0DTE only when that Wednesday is a holiday and the
    expiry rolls back onto Tuesday). When no real Wednesday-expiry option bars
    exist (individual equities often list only Friday weeklies) it falls back to
    Black-Scholes simulation. The time-frame engine then aggregates those minutes
    into arbitrary frames and searches for the optimal intraday entry/exit
    ...
    def _capture_ticker(self, ticker: str) -> None:
        past_dates = get_past_tuesday_dates(self.config.backtest_days)
        ...
        for tuesday in past_dates:
            # The expiry we trade: the next-day Wednesday (1DTE), or the Tuesday
            # itself when that Wednesday is an NYSE holiday (0DTE roll-back).
            exp_date = weekly_expiry_for(tuesday)
            win_start = window_minute_utc(tuesday, start_m)
            win_end = window_minute_utc(tuesday, end_m)

            # Closed Tuesday (holiday) → no session this week.
            if not is_trading_day(tuesday):
                for ot in opt_types:
                    self.daily_capture[(ticker, ot)].append({
                        'date': tuesday, 'strike': 0.0, 'contract': '',
                        'source': SOURCE_NO_STOCK, 'spot_open': 0.0, 'prices': {},
                        'note': 'missing data (Tuesday market holiday)',
                    })
                continue
            ...
            if stock_bars is None or stock_bars.empty:
                for ot in opt_types:
                    self.daily_capture[(ticker, ot)].append({
                        'date': tuesday, 'strike': 0.0, 'contract': '',
                        'source': SOURCE_NO_STOCK, 'spot_open': 0.0, 'prices': {},
                        'note': 'missing data (no Tuesday stock bars)',
                    })
                continue
            ...
            for ot in opt_types:
                ...
                for atm_strike in [lower_strike, upper_strike]:
                    ...
                    self.daily_capture[(ticker, ot)].append({
                        'date': tuesday,            # bucket by the Tuesday
                        'strike': atm_strike,
                        'contract': contract_sym,   # encodes the Wednesday expiry used
                        'source': source,
                        'spot_open': spot_open,
                        'prices': dict(minute_prices),
                        'note': ('0DTE (Wed holiday → Tue expiry)'
                                 if exp_date == tuesday else '1DTE (Wed expiry)'),
                    })
                    captured_any = True
    ...
    def _simulate_option_prices(self, stock_bars: pd.DataFrame, strike: float,
        ...
        trading_hours_until_close), so a 1DTE Wednesday-expiry option keeps its
