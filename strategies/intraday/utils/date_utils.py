...
def get_past_weekday_dates(weekday: int, days_back: int = 150) -> list[date]:
    """All dates of the given `weekday` (Mon=0 … Fri=4) in the past `days_back`
    calendar days, oldest first. The backtester marks a closed session (holiday)
    as a missing week for provenance.
    """
    today = date.today()
    result = []
    for i in range(1, days_back + 1):
...
def expiry_for(entry: date, expiry_weekday: int) -> date:
    """The expiry date for an intraday entry, given the target `expiry_weekday`
    (Mon=0 … Fri=4) in the SAME week (must be >= the entry's weekday):

        Mon entry, expiry_weekday=Mon → same-day 0DTE
        Tue entry, expiry_weekday=Wed → next-day 1DTE
        Thu entry, expiry_weekday=Fri → next-day 1DTE

    If that target day is a market holiday the expiry rolls back to the previous
    trading day (so a closed Wednesday/Friday expiry falls back one session). When
    no real option bars exist for the resulting expiry the backtester simulates.
    """
    delta = expiry_weekday - entry.weekday()
    if delta < 0:
        delta = 0
    exp = entry + timedelta(days=delta)
    return exp if is_trading_day(exp) else previous_trading_day(exp)
