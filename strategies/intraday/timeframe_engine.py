#!/usr/bin/env python3
"""
Time-frame search engine for the all-days intraday optimal entry/exit study.
The entry weekday, expiry, lookback, and tickers come from the Config the caller
passes (see config.DAY_SPECS); this engine is day-agnostic.
...
def print_heatmaps(ticker, opt_type, cell_best, optimal_cell):
    ...
    _print_grid("3b) TOTAL P&L  ($ per contract, summed over all sessions)",
...
def save_csv(opt_type, method_label, header_extra, rows, summ, frame, ranked,
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        # Per-session detail
        w.writerow(["DETAIL — one row per session (ATM "
...
def run_timeframe(lookback_days, method_label, score_key, eligible, size_fn,
                  file_tag, outlier_max=None, header_extra="", config=None):
    ...
    `config` lets a caller inject a per-day Config (tickers, entry/expiry weekday);
    when omitted a default Config() is used."""
