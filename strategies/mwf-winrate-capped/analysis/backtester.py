...
from utils.date_utils import (
    ET, get_past_mwf_dates, detect_strike_interval, get_atm_strikes,
...
class Backtester:
    """Captures 1-minute 0DTE option prices for every past Monday, Wednesday and
    Friday, for BOTH the ATM call and the ATM put, over the 2:00-4:00 PM ET
    capture window. The by-day engine then slices each weekday to its own
    lookback + trading window and searches for the optimal entry/exit schedule.
    Nothing here picks a schedule — it only fetches data.
    ...
    def __init__(self, fetcher, config: Config):
        ...
        # Thursday). Real-only on that fallback — never simulated. Mon/Wed never
        # fall back (only weekday 4 does).
    ...
    def _capture_ticker(self, ticker: str) -> None:
        ...
        for mwf_date in past_dates:
            ...
            exp_date = mwf_date
            real_only = False
            is_friday = mwf_date.weekday() == 4
            win_start = window_minute_utc(mwf_date, start_m)
            win_end = window_minute_utc(mwf_date, end_m)

            # Stock bars across the capture window to determine the ATM strike.
            ...
            # Only Fridays fall back; Mon/Wed are left as missing.
            if ((stock_bars is None or stock_bars.empty)
                    and self.friday_thursday_fallback and is_friday):
            ...
            if stock_bars is None or stock_bars.empty:
                # Surface the closed day as a "missing data" row for both types.
                note = ('missing data (Fri & Thu closed)' if is_friday
            ...
            for ot in opt_types:
                ...
                # Thursday existed but no REAL option bars → mark the day missing
    ...
    def _bars_to_minute_prices(self, bars: pd.DataFrame) -> dict[int, float]:
        ...
        for ts, row in bars.iterrows():
            ...
            if start_m <= m < end_m:  # within the capture window
