#!/usr/bin/env python
"""
PIPELINE ENTRY POINT — Mon/Wed/Fri 0DTE option threshold-timing study.

A DESCRIPTIVE historical study (no live trading, no orders). TRUE 0DTE ONLY —
enter intraday, the option expires that SAME day — restricted to Mon/Wed/Fri
(no multi-day bridges). For each long option (call and put) in the Mag-7 + ORCL +
AMD universe it measures WHEN the mark reaches a ladder of profit thresholds
(+20…+100%) and HOW LONG that exit window stays open — an entry/exit playbook per
symbol, direction, expiry weekday, entry-time window and moneyness. Lookback is
split by expiry weekday: Friday 730d, Mon/Wed 150d.
...
Data: real Alpaca option data when credentials are present — historical
top-of-book QUOTES if the feed serves them, else last-trade BARS + a modeled
spread, chosen per symbol and labelled honestly. Without credentials it uses a
clearly-labelled SIMULATED (Black-Scholes) fallback so the pipeline never goes
silent. The SIMULATED numbers are NOT a real edge.
...
def _load_data(cfg: Config, log) -> tuple:
    ...
    if reason == "ok":
        log("[data] Alpaca credentials present — fetching real 0DTE option data …")
        raw, label, per_symbol_source = _alp.fetch_universe(cfg, log=log)
        if raw is not None and not raw.empty:
            return _schema.validate_contract(raw, cfg), label, per_symbol_source
...
def run(cfg: Config, out_dir: str, log) -> dict:
    df, source_label, per_symbol_source = _load_data(cfg, log)
    if df.empty:
    ...
    for sym in cfg.tickers:
        ...
        val_agg["per_symbol_dow_sessions"].update(
            validation["regimes"].get("per_symbol_dow_sessions", {}))
    ...
    validation = {
        ...
        "regimes": {"ok": len(val_agg["regime_problems"]) == 0,
                    ...
                    "per_symbol_dow_sessions": val_agg["per_symbol_dow_sessions"]},
    ...
    # run_config.json
    run_config = {
        "scope": "TRUE 0DTE only (same-day expiry), Mon/Wed/Fri; no bridges",
        "universe": cfg.tickers,
        "expiry_dows": ["Mon", "Wed", "Fri"],
        "lookback_days": {"friday": cfg.friday_lookback_days,
                          "mon_wed": cfg.monwed_lookback_days},
        ...
        "per_symbol_data_source": per_symbol_source,
        "per_symbol_dow_sessions": val_agg["per_symbol_dow_sessions"],
    ...
    metrics = {
        "status": "ok",
        "strategy": "mwf-0dte-threshold-timing",
        "data_source": source_label,
        "date_range": {"start": date_min, "end": date_max, "n_sessions": n_sessions},
        "universe": cfg.tickers,
    ...
    log("\n" + "=" * 62)
    log("  MWF 0DTE THRESHOLD-TIMING — done")
...
def main() -> int:
    ...
    try:
        ...
        metrics = {"status": "error", "strategy": "mwf-0dte-threshold-timing",
