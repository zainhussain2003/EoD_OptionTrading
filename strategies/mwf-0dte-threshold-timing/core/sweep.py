...
def run_sweep(df: pd.DataFrame, cfg: Config) -> tuple:
    ...
    for sym, sym_df in df.groupby(c.col_symbol):
        ...
        for trade_date, day_df in sym_df.groupby("_date"):
            # 0DTE ONLY, Mon/Wed/Fri: the target expiry must be the SAME day, and
            # that day must be a configured expiry weekday. No bridges.
            if trade_date.weekday() not in c.expiry_dows:
                continue
            day_tgt = day_df[day_df[c.col_expiry] == trade_date]
            if day_tgt.empty:
                continue
            tgt = trade_date
            regime, dte, is_0dte = _exp.regime_label(trade_date, tgt)  # "<DOW>-0DTE"
            day_dow = _exp.dow(trade_date)
