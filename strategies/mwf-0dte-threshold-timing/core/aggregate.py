...
def bucketed_table(instances: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    for keys, g in instances.groupby(GROUP_KEYS, sort=False):
        ...
        # per-bucket date coverage — makes recent-only 0DTE samples visible.
        dts = g["date"].astype(str)
        base["first_date"] = dts.min()
        base["last_date"] = dts.max()
        base["n_sessions"] = int(g["date"].nunique())
