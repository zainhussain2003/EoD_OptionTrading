...
def make_folds(dates: list, train_weeks: int, test_weeks: int, expanding: bool,
               sessions_per_week: int = 5) -> list:
    """Roll (train_dates, test_dates) folds over sorted session dates.
    `sessions_per_week` maps weeks→sessions: 5 for a full daily study, but 1 for a
    single-expiry-weekday 0DTE combo (one Mon/Wed/Fri session per week)."""
    d = sorted(dates)
    tr = train_weeks * sessions_per_week
    te = test_weeks * sessions_per_week
    folds = []
    start = 0
    while True:
        tr_end = start + tr
...
def run_mode_c(instances: pd.DataFrame, cfg, session_open: int) -> dict:
    ...
    # Include `regime` so each expiry weekday (Mon/Wed/Fri 0DTE) gets its OWN
    # frozen entry schedule and walk-forward — different DOWs also carry different
    # lookback windows, so they must not be pooled.
    combo_keys = ["symbol", "direction", "moneyness", "fill_mode", "regime"]
    trade_rows, perf_rows = [], []

    for kv, sub in instances.groupby(combo_keys, sort=False):
        sym, direction, mny, fm, regime = kv
        dates = sorted(sub["date"].unique())
        # one 0DTE session per week for a single expiry weekday → sessions_per_week=1
        ...
        for T in thresholds:
            ...
            perf_rows.append({
                ...
                "fill_mode": fm, "regime": regime, "T_pct": round(T * 100, 4),
                "n_test_trades": n_tr,
