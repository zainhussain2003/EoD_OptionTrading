# Mon/Wed/Fri 0DTE option threshold-timing study

A **descriptive** historical study (no live trading, no orders). **TRUE 0DTE
only** — you enter intraday and the option **expires that same day** — restricted
to **Monday / Wednesday / Friday**, with **no multi-day bridge holds**. For each
long option — call **and** put — in the Mag-7 + ORCL + AMD universe, it measures
**when** the mark reaches a ladder of profit thresholds (+20 … +100 %) and **how
long** that exit window stays open, producing an entry/exit playbook per symbol,
direction, expiry weekday, entry-time window and moneyness.

**Scope details**
- Only sessions whose **same-day** expiry lands on Mon/Wed/Fri are analyzed.
  An equity without a Mon/Wed same-day expiry (ORCL, AMD) contributes only its
  **Friday** 0DTE sessions — those weekdays are simply absent, never bridged.
- **Lookback is split by expiry weekday:** Friday 0DTE = **730 days** (weeklies
  have years of history); Monday & Wednesday 0DTE = **150 days** (recent listings).
- Per-bucket `first_date / last_date / n_sessions` are reported so a recent-only
  Mon/Wed sample is always visible.
...
## Data
Real Alpaca option data via `data/alpaca_quotes.py`, addressing contracts by
**constructed OCC symbols** (works for expired contracts) and probing only the
**same-day** expiry per session (one probe/session — fast). For each contract it
uses historical **top-of-book quotes** when the feed serves them, else last-trade
**bars + a modeled spread**, chosen **per symbol** and recorded in
`run_config.json` (`per_symbol_data_source`). Without Alpaca credentials it falls
back to a clearly-labelled **SIMULATED** (Black-Scholes) frame so the pipeline
never goes silent — those numbers are **not a real edge**. Moneyness
(ITM/ATM/OTM1/OTM2+) is detected from the data.
