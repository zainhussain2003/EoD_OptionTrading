...
def black_scholes_put(S: float, K: float, T: float, r: float, sigma: float
                      ) -> tuple[float, float, float, float]:
    """Returns (put_price, delta, d1, d2). delta = N(d1) - 1 = -N(-d1)."""
    ...
    try:
        ...
        return max(K - S, 0.0), -0.5, 0.0, 0.0


def calc_pop(S: float, K: float, premium: float, T: float, r: float, sigma: float) -> float:
    """Probability that put expires below breakeven (K - premium)."""
    if T <= 0:
        return 1.0 if S < K - premium else 0.0
    breakeven = K - premium
    if breakeven <= 0:
        # Breakeven at/below zero is unreachable — the stock can't go negative.
        return 0.0
    try:
        sqrt_T = math.sqrt(T)
